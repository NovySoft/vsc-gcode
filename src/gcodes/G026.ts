...
const content = `# G26 - Mesh Validation Pattern
## Test the mesh and adjust.
### Usage
\`\`\`plaintext
 G26  [B<temp>] [C<bool>] [D<flag>] [F<linear>] [H<linear>] [I<index>] [K<bool>] [L<linear>] [O<linear>] [P<linear>] [Q<float>] [R<int>] [S<float>] [U<linear>] [X<linear>] [Y<linear>]
\`\`\`

- \`[B<temp>]\`: Bed temperature (otherwise 60°C) to use for the test print.
- \`[C<bool>]\`: Continue with the closest point (otherwise, don't)
- \`[D<flag>]\`: Disable leveling compensation (otherwise, enable)
